...
Observed browser measurements, Lighthouse simulated timing, and instrumented diagnostics remain separate. Missing samples stay missing. Field INP is unavailable. Optional first-repeat screenshots differed only after the initial snapshot: initial metrics remain matched, while later first-repeat interactions/readiness are not strictly matched. Do not present descriptive interaction medians as a strictly controlled responsiveness speedup. Scored Lighthouse runs with page-load time-limit warnings are explicitly counted; results and late-loading diagnostics may be incomplete. Warning descriptions are fixed allowlisted phrases rather than raw text. Timeline event durations may overlap and must not be added into a unique CPU total.
